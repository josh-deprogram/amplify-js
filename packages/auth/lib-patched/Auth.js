											...
											return __awaiter(_this, void 0, void 0, function() {
												...
												return __generator(this, function(_b) {
													switch (_b.label) {
														...
														case 2:
															clientMetadata = this._config.clientMetadata;
															(_a = session.getAccessToken().decodePayload()
																.scope),
																(scope = _a === void 0 ? '' : _a);
															if (scope.split(' ').includes(USER_ADMIN_SCOPE)) {
																user.getUserData(
																	function(err, data) {
																		if (err) {
																			...
																			if (
																				...
																				err.message ===
																					'User does not exist.' ||
																				err.message ===
																					'Access Token has been revoked' // Session revoked by another app
																			) {
																				rej(err);
																			} else {
																		...
																		var attributes = _this.attributesToObject(
																			attributeList
																		);
																		Object.assign(user, {
				...
				switch (_b.label) {
					...
					case 5:
						...
						if (isCustomStateIncluded) {
							...
							dispatchAuthEvent(
								'customOAuthState',
								core_1.urlSafeDecode(customState),
								...
								currUser.setDeviceStatusRemembered({
									...
									onFailure: function(err) {
										if (err.code === 'InvalidParameterException') {
											rej(
												new Errors_1.AuthError(
													Auth_1.AuthErrorTypes.DeviceConfig
												)
											);
										} else if (err.code === 'NetworkError') {
											rej(
												new Errors_1.AuthError(
													Auth_1.AuthErrorTypes.NetworkError
												)
											);
										} else {
											rej(err);
								...
								var cb = {
									onSuccess: function(data) {
										var deviceList = data.Devices.map(function(device) {
											var deviceName =
												device.DeviceAttributes.find(function(_a) {
													var Name = _a.Name;
